...
void
cxpr_print (FILE * stream, struct cxpr z, int sc_not, int sign, int lim)
{
...
void
cxprcxpr (struct cxpr z, int lim)
{
  cxpr_print (stdout, z, 1, 0, lim);
}

void
cxprint (FILE * stream, struct cxpr z)
{
  int i;
...
int
cxfout (FILE * fp, struct xoutflags ofs, struct cxpr z)
{
  if (ofs.fmt < 0)
    ofs.fmt = XFMT_STD;
  else if (ofs.fmt > 2)
...
int
cxout (struct xoutflags ofs, struct cxpr z)
{
