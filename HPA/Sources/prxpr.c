...
static int
printed_special_value (FILE* stream, struct xpr u, int sign)
{
  if ((xisPinf(&u)))
    {
      if ((sign))
...
static int
copied_special_value (char* s, struct xpr u, int sign)
{
  if ((xisPinf(&u)))
    {
      if ((sign))
...
void
xpr_print (FILE * stream, struct xpr u, int sc_not, int sign, int lim)
{
  char q[5 * XDIM + 4];
  register char *p = q;
  register int k, m;
...
char *
xtoa (struct xpr u, int lim)
{
  return xpr_asprint (u, 1, 0, lim);
}

void
xbprint (FILE * stream, struct xpr u)
{
  register int i;
...
void
xprint (FILE * stream, struct xpr u)
{
  register int i;
...
int
xfout (FILE * fp, struct xoutflags ofs, struct xpr x)
{
  char *str;
  int errcode;
...
int
xout (struct xoutflags ofs, struct xpr x)
{
  char *str;
  int errcode;
